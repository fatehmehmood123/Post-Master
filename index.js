...
addParam.addEventListener("click", () => {
  ...
  let string = ` <div class="row my-2 deleteExtraParameters">
                        <legend for="radio" class="col-sm-2 col-form-label">Parameter ${
                          addedParamCount + 2
                        }</legend>
                        <div class="col">
  ...
  if (contentType == "params") {
    ...
    for (let index = 0; index < addedParamCount + 1; index++) {
      if (document.getElementById("parameterKey" + (index + 1)) != undefined) {
        var key = document.getElementById("parameterKey" + (index + 1)).value;
...
//   console.log(JSON.parse(data));
  ...
  if (requestType == "GET") {
    ...
    formdata.append(document.getElementById("parameterKey" + (index+1)).value, document.getElementById(
        "parameterValue" + (index+1)
      ).value)
        }
    fetch(url, {
        ...
        .then((result) => {
            document.getElementById("responsePrism").innerHTML = result;
            Prism.highlightAll();
